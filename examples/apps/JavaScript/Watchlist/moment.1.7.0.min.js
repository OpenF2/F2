...
(function (a, b) {
	...
	function N(b, c) {
		...
		return (
			...
			(e = new a(0)),
			c
				? (e.setUTCFullYear(b[0], b[1], b[2]),
				  e.setUTCHours(b[3], b[4], b[5], b[6]))
				: (e.setFullYear(b[0], b[1], b[2]), e.setHours(b[3], b[4], b[5], b[6])),
			(e._a = b),
			e
		);
	}
	function O(a, b) {
		...
		for (d = 0; d < 12; d++)
			(e = c([2e3, d])),
				(f[d] = new RegExp(
					'^' +
						(b.months[d] || b.months(e, '')) +
						'|^' +
						(b.monthsShort[d] || b.monthsShort(e, '')).replace('.', ''),
					'i'
				));
	...
	function Q(a) {
		return D[a]
			? "'+(" + D[a] + ")+'"
			: a.replace(n, '').replace(/\\?'/g, "\\'");
	}
	function R(a) {
		return P().longDateFormat[a] || a;
	}
	function S(a) {
		var b = "var a,b;return '" + a.replace(l, Q) + "';",
			c = Function;
