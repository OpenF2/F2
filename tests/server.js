...
app.all('/F2/apps/test/hello-world', function (req, res) {
	...
	res.setHeader('Content-Type', 'text/javascript');
	res.send(
		getResponse(req, 'com_openf2_examples_javascript_helloworld', manifest)
	);
});
...
app.all('/F2/apps/test/hello-world-node', function (req, res) {
	...
	var manifest = {
		inlineScripts: [],
		scripts: [
			`${baseUrl}/tests/apps/com_openf2_examples_nodejs_helloworld/appclass.js`
		],
		styles: [
			`${baseUrl}/tests/apps/com_openf2_examples_nodejs_helloworld/app.css`
		],
		apps: [
			{
